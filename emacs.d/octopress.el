...
(defun octopress-rake (command)
  "run rake commands"
  (let ((command-str (format "bash -l -c 'eval \"$(rbenv init -)\" && cd %s && rake %s'" octopress-workdir command)))
...
(defalias 'rakegen  'octopress-generate)
(defalias 'octopre  'octopress-preview)
(defalias 'rakedep  'octopress-deploy)
